...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('job_queue', function (Blueprint $table) {
            ...
            $table->index(['status', 'date_entered'], 'idx_status_entered');
            $table->index(['status', 'date_modified'], 'idx_status_modified');
            $table->index(['status', 'scheduler_id'], 'idx_status_scheduler');
            $table->index(['status', 'execute_time', 'date_entered'], 'idx_status_time');
            $table->timestamps();
            $table->softDeletes();
        });
    }
