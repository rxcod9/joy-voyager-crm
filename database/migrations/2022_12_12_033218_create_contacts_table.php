...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('contacts', function (Blueprint $table) {
            ...
            $table->index(['deleted', 'reports_to_id', 'last_name'], 'idx_cont_del_reports');
            $table->index(['last_name', 'first_name', 'deleted_at'], 'idx_cont_last_first');
            $table->index(['deleted', 'last_name'], 'idx_contacts_del_last');
            $table->index(['deleted', 'id', 'assigned_user_id'], 'idx_del_id_user');
            $table->timestamps();
            $table->softDeletes();
        });
    }
