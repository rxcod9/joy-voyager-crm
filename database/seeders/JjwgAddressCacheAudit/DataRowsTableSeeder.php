...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $dataType = Voyager::model('DataType')->where('slug', 'jjwg-address-cache-audit')->firstOrFail();
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                ...
                'order'        => ++$order,
                'details'      => [
                    'model'       => Voyager::modelClass('JjwgAddressCache'),
                    'table'       => 'jjwg_address_cache',
                    'type'        => 'belongsTo',
                    'column'      => 'parent_id',
                    'key'         => 'id',
                    'label'       => 'name',
                    'pivot_table' => 'jjwg_address_cache',
                    'pivot'       => 0,
                ],
            ])->save();
        }
