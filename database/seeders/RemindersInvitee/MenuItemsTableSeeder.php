...
class MenuItemsTableSeeder extends Seeder
{
    ...
    public function run($parentMenuId = null)
    {
        ...
        $menuItem = Voyager::model('MenuItem')->firstOrNew([
            'menu_id' => $menu->id,
            'title'   => __('joy-voyager-crm::seeders.menu_items.reminders_invitees'),
            'url'     => '',
            'route'   => 'voyager.reminders-invitees.index',
        ]);
        if (!$menuItem->exists) {
            $menuItem->fill([
                'target'     => '_self',
                'icon_class' => 'voyager-bread voyager-crm-reminders_invitee voyager-group',
                'color'      => null,
