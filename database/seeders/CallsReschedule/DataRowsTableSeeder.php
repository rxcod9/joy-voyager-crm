...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $dataType = Voyager::model('DataType')->where('slug', 'calls-reschedules')->firstOrFail();
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => __('joy-voyager-crm::seeders.data_rows.reason'),
                'required'     => 0,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($dataType, 'calls_reschedule_belongsto_call_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'calls_reschedule_belongsto_assigned_user_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'calls_reschedule_belongsto_created_by_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'calls_reschedule_belongsto_modified_by_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
