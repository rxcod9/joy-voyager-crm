...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $dataType = Voyager::model('DataType')->where('slug', 'surveyquestions')->firstOrFail();
        ...
        $dataRow = $this->dataRow($dataType, 'happiness_question');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'checkbox',
                'display_name' => __('joy-voyager-crm::seeders.data_rows.happiness_question'),
                'required'     => 0,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($dataType, 'surveyquestion_belongsto_survey_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'surveyquestion_belongsto_assigned_user_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'surveyquestion_belongsto_created_by_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'surveyquestion_belongsto_modified_by_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
