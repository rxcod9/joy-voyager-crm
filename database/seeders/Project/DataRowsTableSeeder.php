...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $dataType = Voyager::model('DataType')->where('slug', 'projects')->firstOrFail();
        ...
        $dataRow = $this->dataRow($dataType, 'project_belongstomany_cases_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'project_belongstomany_aos_products_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
                'display_name' => __('joy-voyager-crm::seeders.data_rows.aos_products'),
                'required'     => 0,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
                ...
                'details'      => [
                    ...
                    'key'               => 'id',
                    'label'             => 'name',
                    'pivot_table'       => 'projects_products',
                    'foreign_pivot_key' => 'project_id',
                    'related_pivot_key' => 'product_id',
                    'pivot'             => '1',
                    'taggable'          => '0',
                ],
            ])->save();
        ...
        $dataRow = $this->dataRow($dataType, 'project_belongsto_assigned_to_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'project_belongsto_modified_by_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
