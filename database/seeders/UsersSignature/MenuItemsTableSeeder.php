...
class MenuItemsTableSeeder extends Seeder
{
    ...
    public function run($parentMenuId = null)
    {
        ...
        $menuItem = Voyager::model('MenuItem')->firstOrNew([
            'menu_id' => $menu->id,
            'title'   => __('joy-voyager-crm::seeders.menu_items.users_signatures'),
            'url'     => '',
            'route'   => 'voyager.users-signatures.index',
        ]);
        if (!$menuItem->exists) {
            $menuItem->fill([
                'target'     => '_self',
                'icon_class' => 'voyager-bread voyager-crm-users_signature voyager-pen',
                'color'      => null,
