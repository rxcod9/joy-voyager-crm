...
class TranslationsTableSeeder extends Seeder
{
    ...
    private function emailTemplatesTranslations()
    {
        ...
        //
        $cat = Voyager::model('EmailTemplate')->where('name', 'email_template-1')->first();
        if ($cat->exists) {
        ...
        $cat = Voyager::model('EmailTemplate')->where('name', 'email_template-2')->first();
        if ($cat->exists) {
    ...
    private function menusTranslations()
    {
        ...
        $_item = $this->findMenuItem(__('joy-voyager-crm::seeders.menu_items.email_templates'));
        if ($_item->exists) {
