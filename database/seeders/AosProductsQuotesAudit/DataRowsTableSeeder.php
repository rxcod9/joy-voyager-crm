...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $dataType = Voyager::model('DataType')->where('slug', 'aos-products-quotes-audit')->firstOrFail();
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                ...
                'order'        => ++$order,
                'details'      => [
                    'model'       => Voyager::modelClass('AosProductsQuote'),
                    'table'       => 'aos_products_quotes',
                    'type'        => 'belongsTo',
                    'column'      => 'parent_id',
                    'key'         => 'id',
                    'label'       => 'name',
                    'pivot_table' => 'aos_products_quotes',
                    'pivot'       => 0,
                ],
            ])->save();
        }
