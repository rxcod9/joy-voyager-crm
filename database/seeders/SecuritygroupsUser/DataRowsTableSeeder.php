...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $dataType = Voyager::model('DataType')->where('slug', 'securitygroups-users')->firstOrFail();
        ...
        $dataRow = $this->dataRow($dataType, 'securitygroups_user_belongsto_securitygroup_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'securitygroups_user_belongsto_user_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'primary_group');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'checkbox',
                'display_name' => __('joy-voyager-crm::seeders.data_rows.primary_group'),
                'required'     => 0,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
