...
class MenuItemsTableSeeder extends Seeder
{
    ...
    public function run($parentMenuId = null)
    {
        ...
        $menuItem = Voyager::model('MenuItem')->firstOrNew([
            'menu_id' => $menu->id,
            'title'   => __('joy-voyager-crm::seeders.menu_items.aos_pdf_templates'),
            'url'     => '',
            'parent_id'  => $parentMenuId,
            'route'   => 'voyager.aos-pdf-templates.index',
        ]);
        if (!$menuItem->exists) {
            $menuItem->fill([
                'target'     => '_self',
                'icon_class' => 'voyager-bread voyager-crm-aos_pdf_template voyager-file-text',
                'color'      => null,
                'parent_id'  => $parentMenuId,
                'order'      => ++$maxOrder,
            ])->save();
