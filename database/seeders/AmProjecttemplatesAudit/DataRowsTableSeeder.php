...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $dataType = Voyager::model('DataType')->where('slug', 'am-projecttemplates-audit')->firstOrFail();
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                ...
                'order'        => ++$order,
                'details'      => [
                    'model'       => Voyager::modelClass('AmProjecttemplate'),
                    'table'       => 'am_projecttemplates',
                    'type'        => 'belongsTo',
                    'column'      => 'parent_id',
                    'key'         => 'id',
                    'label'       => 'name',
                    'pivot_table' => 'am_projecttemplates',
                    'pivot'       => 0,
                ],
            ])->save();
        }
