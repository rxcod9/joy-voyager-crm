...
class MenuItemsTableSeeder extends Seeder
{
    ...
    public function run($parentMenuId = null)
    {
        ...
        $menuItem = Voyager::model('MenuItem')->firstOrNew([
            'menu_id' => $menu->id,
            'title'   => __('joy-voyager-crm::seeders.menu_items.user_preferences'),
            'url'     => '',
            'route'   => 'voyager.user-preferences.index',
        ]);
        if (!$menuItem->exists) {
            $menuItem->fill([
                'target'     => '_self',
                'icon_class' => 'voyager-bread voyager-crm-user_preference voyager-categories',
                'color'      => null,
