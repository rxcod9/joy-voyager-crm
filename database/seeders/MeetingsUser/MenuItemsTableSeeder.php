...
class MenuItemsTableSeeder extends Seeder
{
    ...
    public function run($parentMenuId = null)
    {
        ...
        $menuItem = Voyager::model('MenuItem')->firstOrNew([
            'menu_id' => $menu->id,
            'title'   => __('joy-voyager-crm::seeders.menu_items.meetings_users'),
            'url'     => '',
            'parent_id'  => $parentMenuId,
            'route'   => 'voyager.meetings-users.index',
        ]);
        if (!$menuItem->exists) {
            $menuItem->fill([
                'target'     => '_self',
                'icon_class' => 'voyager-bread voyager-crm-meetings_user voyager-people',
                'color'      => null,
                'parent_id'  => $parentMenuId,
                'order'      => ++$maxOrder,
            ])->save();
